...
class StudentHistoryRemoteDataSourceImpl
  ...
  Future<Either<ErrorModel,List<QuranTasimi3History>>> getQuranHistory({
   ...
   final result=await ApiService.instance.makeRequest(method: ApiMethod.get, endPoint: ApiManager.QuranHistoryApi+studentId);

 return  result.fold(
    (l){
      return Left(ErrorModel(message: "some thing went wrong in tasmu3 quran student"));
    },
    (r){

      List<QuranTasimi3History> list=List.generate(r["data"].length, (index)=>QuranTasimi3History.fromMap(r["data"][index]
      ));
      return Right(list);
  ...
  Future<Either<ErrorModel,List<HadithHistoryTasmi3>>> getHadithHistory({
    ...
    final result=await ApiService.instance.makeRequest(method: ApiMethod.get, endPoint: ApiManager.HadithHistoryApi+studentId);
  ...
  return result.fold(
    ...
    (r){
      ...
      List<HadithHistoryTasmi3> list=List.generate(r["data"].length, (index)=>HadithHistoryTasmi3.fromMap(r["data"][index]
      ));
      return Right(list);
